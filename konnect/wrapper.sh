...
# TODO the second yq command override the output of the first (and the first actually already overrides the example data)
...
yq -y ".clients |= [{\"id\": \"grapi-explorer.js\", \"name\": \"Grapi Explorer\", \"application_type\": \"web\", \"trusted\": true, \"insecure\": true, \"redirect_uris\": [\"http://$FQDNCLEANED:3000/\"]}]" $CONFIG_JSON | sponge /kopano/ssl/konnectd-identifier-registration.yaml
